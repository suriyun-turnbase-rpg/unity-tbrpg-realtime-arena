namespace RealtimeArena.Room
{
    public static class GameRoomConsts
    {
        public const string ROOM_NAME = "GAME";
        public const string OPTION_TITLE = "title";
        public const string OPTION_PASSWORD = "password";
        public const string OPTION_PLAYER_ID = "playerId";
        public const string OPTION_ACCESS_TOKEN = "accessToken";
        public const int MAX_PASSWORD_LENGTH = 9;
        public const int ENTER_GAME_COUNT_DOWN = 5;
    }
}
